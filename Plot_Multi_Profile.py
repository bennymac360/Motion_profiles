...
(Multiprofile • Move/Wait steps • Waypoint speed cap • Per-step accel/decel • Per-step jerk)
----------------------------------------------------------------------------------------------------
...
Steps table (≤10 rows). Each row is a **Step**:
  • Step Type = Move:
      ...
      - Accel a⁺ (opt) [m/s²] ...... per-step **acceleration** cap; a_step = min(|a⁺|, a_max) if provided (>0)
      - Decel d⁻ (opt) [m/s²] ...... per-step **deceleration** cap; d_step = min(|d⁻|, d_max) if provided (>0)
        (Global caps remain hard; per-row only tightens each side independently.)
...
# ──────────────────────────────────────────────────────────────────────────────
# Multi-profile planner  (Move/Wait + per-step jerk + per-step accel/decel)
# ──────────────────────────────────────────────────────────────────────────────
def plan_multiprofile(x0, v0, steps, vmax, amax, dmax, jmax):
    """
    steps: list of dicts. Each dict is either
      {'type':'move','xf':float,'v_speed':float,'a_plus':float|None,'d_minus':float|None,'j_pref':float|None}
      {'type':'wait','dt':float}
    ...
    Rules:
      ...
      - Per-step accel/decel caps (positive magnitudes) independently tighten a_max / d_max.
    ...
    for i, st in enumerate(steps):
        ...
        if stype == "move":
            ...
            processed.append({
                ...
                "vf": vf_signed,
                "vcap": vcap,
                "a_plus": st.get("a_plus"),
                "d_minus": st.get("d_minus"),
    ...
    for st in processed:
        if st["type"] == "move":
            # Per-step accel/decel tightening (positive magnitudes)
            a_use = amax
            d_use = dmax
            a_pl = st.get("a_plus")
            d_mn = st.get("d_minus")
            if a_pl is not None and math.isfinite(a_pl) and a_pl > 0:
                a_use = min(abs(a_pl), amax)
            if d_mn is not None and math.isfinite(d_mn) and d_mn > 0:
                d_use = min(abs(d_mn), dmax)
            ...
            plan = plan_profile(x_curr, st["xf"], v_curr, st["vf"],
                                vmax_global=vmax, a_max=a_use, d_max=d_use, j_max=j_use,
                                v_cap=st["vcap"])
            plans.append(plan)
            x_curr, v_curr = st["xf"], st["vf"]
    ...
    for idx, (st, plan) in enumerate(zip(processed, plans), start=1):
        if st["type"] == "wait":
            ...
            meta = [f"cap={cap:.6g}"]
            if st.get("a_plus") is not None:
                meta.append(f"a+={st['a_plus']:g}")
            if st.get("d_minus") is not None:
                meta.append(f"d-={st['d_minus']:g}")
            if st.get("j_pref") is not None:
                meta.append(f"j={st['j_pref']:g}")
            # If S-curve, include actual J used
            if plan.get("params"):
                meta.append(f"J_used={plan['params']['J']:.6g}")
            lines.append(
...
def synthesize_multiprofile(x0, v0, plans, samples=2000):
    ...
    v = np.concatenate(v_all) if v_all else np.full_like(t, v0)
    a = np.concatenate(a_all) if a_all else np.zeros_like(t)
    j = np.concatenate(j_all) if j_all else np.zeros_like(t)
...
class MotionApp(ttk.Frame):
    ...
    def _build_form(self, parent):
        ...
        self.vars = {
            ...
            "amax": tk.StringVar(value="10000.0"),
            "dmax": tk.StringVar(value="10000.0"),
            "jmax": tk.StringVar(value="100000.0"),
        ...
        ttk.Separator(frm, orient="horizontal").grid(row=3, column=0, columnspan=4, sticky="ew", pady=(6, 6))
        ...
        ttk.Separator(frm, orient="horizontal").grid(row=10, column=0, columnspan=4, sticky="ew", pady=(6, 6))
        ...
        mp = ttk.LabelFrame(
            ...
            text=(
                ...
                "Move: Target x [m], Target v (speed) [m/s], Accel a⁺ (opt) [m/s²], Decel d⁻ (opt) [m/s²], Jerk j (opt) [m/s³].\n"
                ...
                "  • a⁺ and d⁻ are positive magnitudes; each independently tightens its global limit if provided (>0).\n"
        ...
        mp.grid(row=11, column=0, columnspan=4, sticky="ew", pady=(2, 8))
        for c in range(11):
        ...
        ttk.Label(mp, text="Accel a⁺ (opt) [m/s²]").grid(row=0, column=5, padx=2)
        ttk.Label(mp, text="Decel d⁻ (opt) [m/s²]").grid(row=0, column=6, padx=2)
        ttk.Label(mp, text="Jerk j (opt) [m/s³]").grid(row=0, column=7, padx=2)
        ttk.Label(mp, text="Wait t [s]").grid(row=0, column=8, padx=2)
        ...
        for i in range(10):
            ...
            a_plus_var = tk.StringVar(value="")
            d_minus_var = tk.StringVar(value="")
            ...
            e_ap = ttk.Entry(mp, textvariable=a_plus_var, width=12, justify="right")
            e_dm = ttk.Entry(mp, textvariable=d_minus_var, width=12, justify="right")
            ...
            e_ap.grid(row=r, column=5, padx=2, pady=1)
            e_dm.grid(row=r, column=6, padx=2, pady=1)
            e_j.grid(row=r, column=7, padx=2, pady=1)
            e_t.grid(row=r, column=8, padx=2, pady=1)

            def make_on_type_change(entries_move=(e_x, e_v, e_ap, e_dm, e_j), entry_wait=e_t, var=type_var):
                def _on_change(*_):
            ...
            self.mp_rows.append({
                ...
                "xf": x_var, "vf": v_var, "ap": a_plus_var, "dm": d_minus_var, "jf": j_var, "wt": t_var
            })

        ttk.Separator(frm, orient="horizontal").grid(row=12, column=0, columnspan=4, sticky="ew", pady=(6, 6))
        ...
        ttk.Checkbutton(frm, text="Use LaTeX (requires TeX)", variable=self.use_latex_var)\
            .grid(row=14, column=0, columnspan=4, sticky="w", pady=(8, 0))

        btns = ttk.Frame(frm)
        btns.grid(row=15, column=0, columnspan=4, sticky="w", pady=(10, 0))
        ...
        ttk.Separator(frm, orient="horizontal").grid(row=16, column=0, columnspan=4, sticky="ew", pady=(10, 6))
        ttk.Label(frm, text="Summary", font=("Segoe UI", 11, "bold")).grid(row=17, column=0, columnspan=4, sticky="w")

        # Scrollable Summary
        sumfrm = ttk.Frame(frm)
        sumfrm.grid(row=18, column=0, columnspan=4, sticky="nsew", pady=(4, 0))
        frm.rowconfigure(18, weight=1)  # allow summary area to grow vertically

        self.txt_summary = tk.Text(sumfrm, width=70, height=18, wrap="word", relief="solid", borderwidth=1)
        scrolly = ttk.Scrollbar(sumfrm, orient="vertical", command=self.txt_summary.yview)
        self.txt_summary.configure(yscrollcommand=scrolly.set)
        self.txt_summary.grid(row=0, column=0, sticky="nsew")
        scrolly.grid(row=0, column=1, sticky="ns")
        sumfrm.columnconfigure(0, weight=1)
        sumfrm.rowconfigure(0, weight=1)
